...
""", text, input.offset)
  ...
  private def expandQuery(start: Seq[StartItem], namedPaths: Seq[NamedPath], updates: Seq[UpdateAction], body: Body): Query = body match {
    case b: BodyWith => {
      ...
      Query(b.returns, start, updates, b.matching, optional = false, Seq(), b.where.getOrElse(True()), b.aggregate,
        b.order, b.slice, b.namedPath ++ namedPaths, Some(expandQuery(b.start, b.startPaths, b.updates, b.next)))
    }
    case b: BodyReturn => {
      checkForAggregates(b.where)
      Query(b.returns, start, updates, b.matching, optional = false, Seq(), b.where.getOrElse(True()), b.aggregate,
        b.order, b.slice, b.namedPath ++ namedPaths, None)
    }
    case NoBody() => {
      Query(Return(Nil), start, updates, Seq(), optional = false, Seq(), True(), None, Seq(), None, namedPaths, None)
    }
  }
