...
class MatchingContextTest extends GraphDatabaseTestBase with Assertions with PatternGraphBuilder {
  ...
  @Test def doubleOptional() {
    val patterns: Seq[Pattern] = Seq(
      RelatedTo(SingleNode("a"), SingleNode("x"), "r1", Seq(), Direction.OUTGOING, optional = true, Map.empty),
      RelatedTo(SingleNode("b"), SingleNode("x"), "r2", Seq(), Direction.OUTGOING, optional = true, Map.empty)
    )
    createMatchingContextWithNodes(patterns, Seq("a", "b"))
  ...
  @Test def solveDoubleOptionalProblem() {
    ...
    val patterns = Seq(
      RelatedTo(SingleNode("a"), SingleNode("x"), "r1", Seq(), Direction.OUTGOING, optional = true, Map.empty),
      RelatedTo(SingleNode("x"), SingleNode("b"), "r2", Seq(), Direction.INCOMING, optional = true, Map.empty)
    )
