...
class ScalaPatternMatchingTest extends ExecutionEngineHelper with PatternGraphBuilder {
  ...
  @Test def should_handle_a_single_relationship_with_no_matches() {
    ...
    val matcher = new PatternMatchingBuilder(patternGraph, Seq.empty, Seq.empty)
  ...
  @Test def should_only_return_matches_that_fulfill_the_uniqueness_constraint() {
    ...
    val matcher = new PatternMatchingBuilder(patternGraph, Seq(), Seq.empty)
    val n0 = createNode()
    val n1 = createNode()
    relate(n0, n1)
  ...
  @Test def should_exclude_matches_overlapping_previously_found_relationships() {
    ...
    val identifiersInMatchClause = Seq("a", "b", "c", "r1", "r2")
    val matcher = new PatternMatchingBuilder(patternGraph, Seq(), identifiersInMatchClause)
  ...
  @Test def should_not_exclude_matches_with_relationships_in_scope_but_outside_clause() {
    ...
    val symbolTable = symbols.add("b", NodeType()).add("r1", RelationshipType())
    val patternGraph = buildPatternGraph(symbolTable, Seq(r2))
    val identifiersInMatchClause = Seq("b", "r2", "c")
    val matcher = new PatternMatchingBuilder(patternGraph, Seq(), identifiersInMatchClause)
  ...
  @Test def shouldReturnTwoSubgraphsWithBoundUndirectedRelationshipAndOptionalRelationship() {
    ...
    val pr1 = RelatedTo("a", "b", "r", Seq.empty, Direction.BOTH)
    val pr2 = RelatedTo(SingleNode("b"), SingleOptionalNode("c"), "r2", Seq.empty, Direction.OUTGOING, optional = true)

    val startingState = ExecutionContext.empty.newWith(Map("r" -> r0))
    val symbolTable = symbols.add("r", RelationshipType())
    val patternGraph = buildPatternGraph(symbolTable, Seq(pr1, pr2))
    val matcher = new PatternMatchingBuilder(patternGraph, Seq.empty, Seq("a", "r", "b", "r2", "c"))

    val result = matcher.getMatches(startingState, queryState).toList

    assertEquals(Set(Map("a" -> n1, "b" -> n2, "c" -> n3, "r" -> r0, "r2" -> r1),
      Map("a" -> n2, "b" -> n1, "c" -> null, "r" -> r0, "r2" -> null)), result.toSet)
  }
}