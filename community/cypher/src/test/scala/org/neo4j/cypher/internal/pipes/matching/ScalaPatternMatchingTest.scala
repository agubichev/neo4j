...
class ScalaPatternMatchingTest extends ExecutionEngineHelper with PatternGraphBuilder {
  ...
  val patternRelationship: RelatedTo = RelatedTo("a", "b", "r", Seq.empty, Direction.OUTGOING, optional = false, True())
  ...
  @Test def should_handle_a_single_relationship_with_no_matches() {
    ...
    val matcher = new PatterMatchingBuilder(patternGraph, Seq.empty)
  ...
  @Test def should_only_return_matches_that_fulfill_the_uniqueness_constraint() {
    ...
    val r1 = RelatedTo("a", "b", "r1", Seq.empty, Direction.BOTH, optional = false, True())
    val r2 = RelatedTo("b", "c", "r2", Seq.empty, Direction.BOTH, optional = false, True())

    val patternGraph = buildPatternGraph(symbols, Seq(r1, r2))
    val matcher = new PatterMatchingBuilder(patternGraph, Seq())
    val n0 = createNode()
    val n1 = createNode()
    relate(n0, n1)
  ...
  @Test def shouldReturnTwoSubgraphsWithBoundUndirectedRelationshipAndOptionalRelationship() {
    ...
    val r1 = RelatedTo("a", "b", "r", Seq.empty, Direction.BOTH, optional = false, True())
    val r2 = RelatedTo("b", "c", "r2", Seq.empty, Direction.OUTGOING, optional = true, True())
