...
public class AllNodeScanOperator extends Operator
{
    ...
    public AllNodeScanOperator( FrameSlot slot, FrameSlot iteratorSlot )
    {
        this.slot = slot;
    ...
    {
        Iterator<Node> it = GlobalGraphOperations.at( getGraph( frame ) ).getAllNodes().iterator();
    ...
    {
        Iterator<Node> it = (Iterator<Node>) FrameUtil.getObjectSafe( frame, iteratorSlot );
        if ( it.hasNext() )
        {
            frame.setLong( slot, it.next().getId() );
            return true;
        }
        return false;
    }
