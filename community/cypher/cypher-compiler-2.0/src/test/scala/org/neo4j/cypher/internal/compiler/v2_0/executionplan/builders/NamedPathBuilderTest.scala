...
class NamedPathBuilderTest extends BuilderTest {
  ...
  def should_accept_if_pattern_is_solved() {
    ...
    val q = PartiallySolvedQuery().
      copy(start = Seq(Solved(NodeById("l", 0))),
      patterns = Seq(Solved(RelatedTo(SingleNode("l"), SingleNode("r"), "rel", Seq(), Direction.OUTGOING, NoProperties))),
  ...
  def should_not_accept_unless_all_parts_of_the_named_path_are_solved() {
    val q = PartiallySolvedQuery().
      copy(start = Seq(Solved(NodeById("l", 0))),
      patterns = Seq(
        Solved(RelatedTo(SingleNode("l"), SingleNode("r"), "rel", Seq(), Direction.OUTGOING, NoProperties)),
        Unsolved(RelatedTo(SingleNode("r"), SingleNode("x"), "rel2", Seq(), Direction.OUTGOING, NoProperties))
      ),
