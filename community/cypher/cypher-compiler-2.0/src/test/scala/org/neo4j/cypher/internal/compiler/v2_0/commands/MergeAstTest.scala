...
class MergeAstTest extends Assertions {
  ...
  def simple_node_without_labels_or_properties() {
    // given
    val from = mergeAst(patterns = Seq(ParsedEntity(A, Identifier(A), NoProperties, Seq.empty, bare = true)))

    // then
    assert(from.nextStep() === Seq(MergeNodeAction(A, NoProperties, Seq.empty, Seq.empty, Seq.empty, Seq.empty, None)))
  }

  @Test
  def node_with_labels() {
    // given
    val from = mergeAst(patterns = Seq(ParsedEntity(A, Identifier(A), NoProperties, Seq(KeyToken.Unresolved(labelName, Label)), bare = true)))
  ...
  def node_with_properties() {
    // given
    val from = mergeAst(patterns = Seq(ParsedEntity(A, Identifier(A), ExpressionMap(propertyKey.name -> expression), Seq.empty, bare = true)))

    assert(from.nextStep() === Seq(MergeNodeAction(A,
      props = ExpressionMap(propertyKey.name -> expression),
      labels = Seq.empty,
  ...
  def node_with_on_create() {
    ...
    val from = mergeAst(
      patterns = Seq(ParsedEntity(A, Identifier(A), NoProperties, Seq.empty, bare = true)),
