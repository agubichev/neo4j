...
class PlanGeneratorTest extends Assertions with MockitoSugar {
  ...
  val costEstimator = mock[CardinalityEstimator]
  val calculator = mock[CostCalculator]
  val generator = new PlanGenerator(costEstimator, calculator)
  val personLabelId: Int = 1337
  val personLabelToken: LabelToken = LabelToken(personLabelId)
  ...
  @Test def simpleLabeledPattern() {
    ...
    when(costEstimator.cardinalityForScan(personLabelToken)).thenReturn(100)

    // When
    val result = generator.generatePlan(planContext, queryGraph)

    // Then
    assert(LabelScan(Id(0), personLabelToken, 100) === result)
  }
  ...
  @Test def simpleRelationshipPatternWithCheaperLabelScan() {
    ...
    // Then
    assert(Expand(Id(0), LabelScan(Id(0), personLabelToken, 100), Direction.OUTGOING, 100 * 5) === result)
  }
}