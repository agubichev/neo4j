...
case class SemanticState(
  ...
  def symbolTypes(name: String) = this.symbol(name).map(_.types).getOrElse(T)
  ...
  def expressionType(expression: ast.Expression): ExpressionTypeInfo = typeTable.getOrElse(expression, ExpressionTypeInfo(T))
