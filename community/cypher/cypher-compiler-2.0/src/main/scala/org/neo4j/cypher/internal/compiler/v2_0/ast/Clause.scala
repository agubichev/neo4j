...
case class Foreach(identifier: Identifier, expression: Expression, updates: Seq[Clause], token: InputToken) extends UpdateClause with SemanticChecking {
  ...
  def semanticCheck =
    ...
    expression.expectType(CTCollectionT) then withScopedState {
      val possibleInnerTypes: TypeGenerator = expression.types(_).reparent { case c: CollectionType => c.innerType }
